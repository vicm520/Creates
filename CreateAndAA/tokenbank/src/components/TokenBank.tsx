...
// tokenbank 合约地址
const TOKEN_BANK_ADDRESS = '0xb23aa17b767667904746C7D98Bfd14E6b4f8ec58'; 
// SimpleDelegateContract 合约地址 (占位符地址)
const SIMPLE_DELEGATE_ADDRESS = '0xA35F4F42A16D33a185226Df83FF2A4BadB6c8B4a';
// 代币 decimals
const TOKEN_DECIMALS = 18; 

const ACCOUNT_PRIVATE_KEY = '私钥';

// 导入 JSON ABI 文件
import tokenABI from '../abi/Token.json';
import tokenBankABI from '../abi/TokenBank.json';
import simpleDelegateABI from '../abi/SimpleDelegateContract.json';
...
export function TokenBank() {
  ...
  const [aaDepositLoading, setAADepositLoading] = useState(false);
  // 错误信息
  ...
  // AA存款操作
  const handleAADeposit = async () => {
    if (!account || !amount || parseFloat(amount) <= 0) {
      setError('请输入有效的金额');
      return;
    }
    setAADepositLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!contractCode || contractCode === '0x') {
        setError('SimpleDelegateContract 合约尚未部署，无法使用 AA 存款功能');
        return;
      }
      ...
      const accountK = privateKeyToAccount(ACCOUNT_PRIVATE_KEY as `0x${string}`);
      ...
      // 调用 SimpleDelegateContract 的 execute 方法
      const nonce = await publicClient.getTransactionCount({
        address: accountK.address,
        blockTag: 'pending'
      });
      const hash = await walletClient.writeContract({
        account: accountK,
      ...
      setSuccessMessage('AA存款成功！');
      fetchBalances(); // 刷新余额
    } catch (err: any) {
      console.error('AA存款失败:', err);
      // setError(
      //   err.message?.includes('user rejected')
      //     ? '用户拒绝了交易'
      //     : 'AA存款失败'
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div className="bg-gradient-to-r from-gray-800 to-gray-900 border border-green-500/30 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="mt-4">
              <button
                onClick={handleAADeposit}
                disabled={aaDepositLoading}
              ...
              >
                <div className="flex items-center justify-center space-x-2">
                  {aaDepositLoading ? (
                    ...
                    <>
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M13 10V3a1 1 0 10-2 0v7a1 1 0 001 1h7a1 1 0 100-2h-7z" clipRule="evenodd" />
                        <path fillRule="evenodd" d="M3 3a1 1 0 011-1h12a1 1 0 011 1v4a1 1 0 11-2 0V4H5v12h8a1 1 0 110 2H4a1 1 0 01-1-1V3z" clipRule="evenodd" />
                      </svg>
                      <span>AA存款</span>
                    </>
                  )}
